...
export class ProductsHomeComponent implements OnInit, OnDestroy{
  ...
  //Da mesma forma que da para usar uma interface no parametro,
  //podemos Criar entre o objeto como tipo do parametro
  ...
  deleteProduct(product_id: string) {
   if(product_id){
    this.productsServices.deleteProduct(product_id)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
          if (response){
          ...
          this.messageService.add({
            severity:'error',
            summary: 'Error',
            detail: 'Não foi possivel remover o produto!',
            life: 2500
          })
        }
      })
